...
def set_path_metrics(metrics, path_name, path, time, oc, start, goal, resolution, expected_path_length):
    ...
    if path is not None:
        ...
        eps = 0.01
        metrics[path_name]['reached_start'] = bool(np.linalg.norm(path[0] - start) <= eps)
        metrics[path_name]['reached_goal'] = bool(np.linalg.norm(path[-1] - goal) <= eps)
        metrics[path_name]['no_collision'] = any([not oc.is_occupied_metric(p) for p in path])

        # Check if the difference between each step is less than the resolution
        # we can't check exactly due to numerical errors, so we need an epsilon 1e-9
        # This still works to get the optimal path since the path must be the same length as
        # the optimal and fit within the resolution bounds.
        diff_path = abs(np.diff(path, axis=0))
        metrics[path_name]['contiguous'] = all([all(abs(d) < (resolution + 1e-9)) for d in diff_path])
    else:
        ...
        metrics[path_name]['contiguous'] = False
    ...
    if expected_path_length is not None:
        if expected_path_length == np.inf and metrics[path_name]['path_length'] == np.inf:
            ...
            metrics[path_name]['is_optimal'] = bool(abs(metrics[path_name]['path_length'] - expected_path_length) <= max(
            resolution))
    else:
...
def test_mission(graph_search_fn, world, start, goal, resolution, margin, expected_path_length):
    ...
    set_path_metrics(metrics, 'dijkstra', results['dijkstra_path'], dijkstra_time, oc, start, goal, resolution, expected_path_length)
    set_path_metrics(metrics, 'astar', results['astar_path'], astar_time, oc, start, goal, resolution, expected_path_length)

    w = np.array(world.world['bounds']['extents'])
    metrics['map_nodes'] = int(np.prod(np.round((w[1::2]-w[0::2])/np.array(resolution))))
        ...
        for file in files:
            if file.stem.startswith('test_'):
                ...
                def fn(self, test_name=test_name,
                       ...
                       margin=data['margin'],
                       expected_path_length=data.get('expected_path_length', None)):
            ...
            if len(r.keys()) > 1:
                if r['test_name'] != 'impossible':
                    ...
                    for name in ['dijkstra', 'astar']:
                        ...
                        print('    {} contiguous:    {}'.format(name, 'pass' if r[name]['contiguous'] else 'FAIL'))
